...
import { activeTemplateKeyAtom, activeThemeAtom, cvDataAtom } from '@/state/atoms'
...
export function ThemeEditor() {
  ...
  const mergedTheme = { ...template.defaultTheme, ...theme } as Record<string, string>

  const updateField = (key: string, value: string) => {
    setTheme({ ...mergedTheme, [key]: value })
  }

  const hasPhotoSettings = template.themeSchema.some((f) => f.key === 'photoShape')
    ...
    <div className="w-full overflow-hidden space-y-4">
      ...
      {hasPhotoSettings && rawPhoto && (
        <PhotoCropper
          ...
          photoShape={(mergedTheme.photoShape as 'round' | 'square' | 'rounded') ?? 'round'}
          onCropDone={(base64) => setTheme({ ...mergedTheme, croppedPhoto: base64 })}
          onCropClear={() => {
